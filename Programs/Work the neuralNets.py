...
from Programs.networkAndFiles import saveToFile, loadFromFile
...
train = np.single(train)
trainRes = np.single(trainRes)
test = np.single(test)
testRes = np.single(testRes)
net.train(train, trainRes, test, testRes, 1, 1)
